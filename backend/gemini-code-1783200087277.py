...
# base_dir points directly to the 'backend' folder where app.py lives
base_dir = os.path.dirname(os.path.realpath(__file__))
template_dir = os.path.join(base_dir, "templates")

app = Flask(
    __name__,
    template_folder=template_dir,  # Look for html files in backend/templates
    static_folder=base_dir,        # Look for static assets in backend/
    static_url_path="",
)
...
"""
    ...
    try:
        res_data = call_gemini_generate(payload, key)
        text = extract_text_from_gemini_response(res_data)
        return jsonify({"success": True, "result": text.strip()})
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500

if __name__ == '__main__':
    # Standard block for running locally
